...
(async () => {
  ...
  await p.evaluateOnNewDocument(() => localStorage.setItem('cornerstone.settings', JSON.stringify({ text: 'large', sound: false, motion: true, speed: 'fast' })));
  ...
  const S = () => p.evaluate(() => { const s = window.__cornerstone.S, w = document.querySelector('#boardwrap');
    return { cell: s.cell, closeUp: s.closeUp, cursor: s.cursor, sel: s.sel && { p: s.sel.p, rot: s.sel.rot, flip: s.sel.flip }, humanTurn: s.humanTurn, over: s.over,
      ...
      bodyHidden: document.querySelector('#hand-body').hidden, boardH: Math.round(w.getBoundingClientRect().height), pageH: document.documentElement.scrollHeight }; });
  ...
  // 1. picking a piece zooms in and places it where it fits
  await p.tap('#tray .piece'); await sleep(300);
  let s = await S();
  ok(s.closeUp && s.cell >= 28, `picking a piece zooms in (square size ${s.cell}px)`);
  ok(/fits/.test(s.status), `the piece starts in a spot where it fits ("${s.status}")`);
  ok(s.pageH <= 740, `no page scrolling (page ${s.pageH}px tall on a 740px screen)`);

  // 2. zoom out; a sloppy tap one square off a legal spot still snaps into place
  await p.tap('#view-toggle'); await sleep(250); s = await S();
  ok(!s.closeUp && s.cell < 20, `Zoom out shows the whole board (${s.cell}px squares)`);
  ...
  // 3. zoom in again, then drag the piece with a finger
  await p.tap('#view-toggle'); await sleep(250); s = await S();
  const g0 = s.cursor;
  ...
  // 6. hide / show controls; menu dialog
  const h0 = s.boardH;
  await p.tap('#hand-toggle'); await sleep(250); s = await S();
  ok(s.bodyHidden && s.boardH > h0, `Hide gives the board more room (${h0}px -> ${s.boardH}px)`);
  await p.tap('#hand-toggle'); await sleep(250); s = await S();
  ...
  ok(!s.closeUp, 'after moves the view zooms back out to the whole board');
